...
# truncated_retweeted_quoted_status = tweet.find({"retweeted_status.quoted_status.truncated": True}) # Find tweets that are truncated in retweeted quoted status object
# for tweet_object in truncated_retweeted_quoted_status: # Loop over all the tweets and replace the truncated tweet with the full text one
#     tweet.update_one({"_id" : tweet_object["_id"]}, {"$set" : {"retweeted_status.quoted_status.text" : tweet_object["retweeted_status"]["quoted_status"]["extended_tweet"]["full_text"]}})

# truncated_retweeted_quoted_status = tweet.find({"retweeted_status.quoted_status.truncated": {"$exists": True}}) # Find tweets that are truncated inside retweeted quoted status object
# for tweet_object in truncated_retweeted_quoted_status: # Exchanges entities from extended tweet to the tweet object for truncated tweets in retweeted quoted status object
#     if tweet_object["retweeted_status"]['quoted_status']['truncated']:
#         tweet.update_one({"_id" : tweet_object["_id"]}, {"$set" : {"retweeted_status.quoted_status.entities" : tweet_object["retweeted_status"]["quoted_status"]["extended_tweet"]["entities"]}})

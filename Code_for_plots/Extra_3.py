...
from index import sentiment_analysis as tweet
...
def occurance_of_words():
    ...
    for tweet in KLM_conversations.find({"depth_1.sentiment" : -1}):
...
def occurance_of_topics():
    ...
    for tweet in KLM_conversations.find({"depth_1.sentiment" : -1, "depth_3" : {"$exists" : True}}):
        for word in tweet["depth_1"]["text"].translate(str.maketrans('', '', string.punctuation)).lower().split():
            for key in topic:
                if word in eval(key):
                    topic[key][0] += 1
...
def sentiment_change_per_topic(topic_occurance_dct):
...
d = occurance_of_topics()
print(sentiment_change_per_topic(d))
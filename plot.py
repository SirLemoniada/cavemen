...
def reply_time_in_hours():
    ...
    for reply_time in klm.find({'reply':{'$exists':True}}):
        reply_time_list_KLM.append((float(reply_time['reply']['timestamp_ms']) -float(reply_time['timestamp_ms']))/1000/60/60)
        
    for reply_time in British_Airways.find({'reply':{'$exists':True}}):
        reply_time_list_British_Airways.append(((float(reply_time['reply']['timestamp_ms']) -float(reply_time['timestamp_ms'])))/1000/60/60)
...
def users_reply_to_init_airline():
    reply_to_KLM = [10,15,6,11,13,12]
    reply_to_British_Airways = [8,7,20,16,5,16,15,17,20]
